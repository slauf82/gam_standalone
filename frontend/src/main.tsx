...
function commercialRows(summary?:InvoiceSummary, totals?:InvoiceTotals, lines?:InvoiceLine[]) {
  ...
  const rows:string[] = [];
  if ((summary.discountPercent ?? 0) > 0 && lineGross > 0) {
    const d = lineGross * (summary.discountPercent ?? 0) / 100;
    adjustedGross -= d;
    rows.push(`Rabatt ${summary.discountPercent}%: -${money(d)}`);
  }
  if ((summary.couponAmount ?? 0) > 0) {
    adjustedGross -= summary.couponAmount ?? 0;
    rows.push(`Gutschein: -${money(summary.couponAmount)}`);
  }
  if (summary.discountRemark) rows.push(summary.discountRemark);
  adjustedGross = Math.max(0, adjustedGross);
  if (rows.length) rows.push(`Endbetrag nach Abzug: ${money(adjustedGross)}`);
  if ((summary.installments ?? 1) > 1) rows.push(`Ratenzahlung: ${summary.installments} Raten à ca. ${money(adjustedGross / (summary.installments ?? 1))}`);
...
function InvoicesPage(){
 ...
 async function refresh(q=filter){if(!searchCompanyId){setRows([]); setSelected(null); setSearchInfo('Bitte zuerst eine Gesellschaft auswählen.'); return;} setSearchInfo(''); const r=await loadInvoices(100,q,searchCompanyId); setRows(r); if(r[0]) setSelected(await loadInvoice(r[0].number, r[0].companyId)); else setSelected(null);}
