...
function InvoicesPage(){
 ...
 useEffect(()=>{loadCompanies().then(cs=>{setCompanies(cs); if(cs[0]) setSearchCompanyId(cs[0].id)}).catch(()=>{}); loadProducts('',200).then(setProducts).catch(()=>{}); loadLbdPreview().then(r=>{setLbd(r); if(r?.found){setRecSalutation(r.salutation||''); setRecTitle(r.title||''); setRecFirstName(r.firstName||''); setRecLastName(r.lastName||''); setRecStreet(r.street||''); setRecPostalCode(r.postalCode||''); setRecCity(r.city||''); setRecCountry(r.country||'Deutschland');}}).catch(()=>{setLbd(null); setRecipientMode('manual');});},[]);
